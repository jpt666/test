...
{
    NSData *imageData = [UIImage dataForCompressImage:image compressRatio:ratio maxCompressRatio:maxRatio];
    //Retuns the compressed image
    return [[UIImage alloc] initWithData:imageData];
}

+ (NSData *)dataForCompressImage:(UIImage *)image compressRatio:(CGFloat)ratio maxCompressRatio:(CGFloat)maxRatio
{
    //We define the max and min resolutions to shrink to
//    int MIN_UPLOAD_RESOLUTION = 1334 * 750;
    int MAX_UPLOAD_SIZE = 150*1024;
//    
//    float factor;
//    float currentResolution = image.size.height * image.scale * image.size.width * image.scale;
//    
...
//        image = [self scaleDown:image withSize:CGSizeMake(image.size.width*image.scale / factor, image.size.height*image.scale / factor)];
//    }
    
//    if (image.size.width>ScreenWidth) {
//        image = [image resizedImage:CGSizeMake(ScreenWidth, image.size.height/(image.size.width/ScreenWidth)) interpolationQuality:kCGInterpolationHigh];
//    }
